...
func initDeleteServer() (*fiber.App, *mock.Container, *mock.Log, *mock.Database, *mock.CrudRepo, *mock.CrudConfig) {
	app := fiber.New()
	c := &mock.Container{}
	log := &mock.Log{}
	db := &mock.Database{}
	...
	app.Delete("/:id", gocruddy.Delete(c, config))

	return app, c, log, db, crudRepo, config
}

func assertAllDeleteExpectations(
	t *testing.T,
	c *mock.Container,
	log *mock.Log,
	db *mock.Database,
...
func TestDelete_IdError(t *testing.T) {
	app, c, log, db, crudRepo, config := initDeleteServer()

	log.On("ErrWarn", mock.Anything).Once()

	req := httptest.NewRequest("DELETE", "http://example.com/asd", nil)
	test.ExpectStatus(t, app, req, fiber.StatusBadRequest)

	assertAllDeleteExpectations(t, c, log, db, crudRepo, config)
}
...
func TestDelete_TransactionError(t *testing.T) {
	...
	req := httptest.NewRequest("DELETE", "http://example.com/123", nil)
	test.ExpectStatus(t, app, req, fiber.StatusInternalServerError)

	assertAllDeleteExpectations(t, c, log, db, crudRepo, config)
}

func TestDelete_GetByIDError_NotFound(t *testing.T) {
	app, c, log, db, crudRepo, config := initDeleteServer()
	...
	config.On("CreateDeleteFilter", mock.Anything).Return(filter).Once()
...
func TestDelete_RemoveError(t *testing.T) {
	...
	crudRepo.On("Remove", mock.Anything, entry).Return(errors.New("test")).Once()
	log.On("ErrError", mock.Anything).Once()
...
func TestDelete_Success(t *testing.T) {
	...
	config.On("GetEntityEntry").Return(entry).Once()
	crudRepo.On("GetByID", mock.Anything, uint(123), mock.AnythingOfType("gocruddy.DatabaseFilter"), entry).Return(entry, nil).Once()
	crudRepo.On("Remove", mock.Anything, entry).Return(nil).Once()
